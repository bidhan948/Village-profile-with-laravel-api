...
@section('setting_relation', 'active')
    ...
    @if (isset($relation))
        <div class="card text-sm mt-1">
            <div class="card-header">
                <div class="card-text">
                    <p class="mb-0">{{ __('नाता सच्याउने') }}</p>
                </div>
                <div class=" card-body">
                    <form method="post" action="{{ route('relation.update', $relation) }}">
                        @csrf
                        @method('PUT')
                        <div class="row" style="margin-left:-30px;">
                            <div class="col-6">
                                <div class="input-group input-group-sm">
                                    ...
                                    <input type="text" value="{{ $relation->name }}" name="name"
                                        class="form-control  @error('name') is-invalid @enderror">
                                    @error('name')
                                        <p class="invalid-feedback" style="font-size: 1rem">
                                            {{ __('नाताको फिल्ड खाली छ ') }}
                                        </p>
                                    @enderror
                                </div>
                            </div>
            ...
            <table id="example1" class="table table-bordered table-striped">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">{{ __('नाता') }}</th>
                        <th></th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    @foreach ($relations as $relation)
                        <tr>
                            ...
                            <td class="text-center">{{ $relation->name }}
                            </td>
                            <td class="text-center"><a href="{{ route('relation.edit', $relation) }}"
                                    class="btn-sm btn-success"><i class="fas fa-edit px-1"></i> {{ __('सच्याउने') }}</a>
                                <a href="#" class="btn-sm btn-danger"
                                    onclick="event.preventDefault();
                                                                                                            document.getElementById('delete_relation{{ $i }}').submit();">
                                    <i class="fas fa-trash-alt px-2"></i>{{ __('हटाउनुहोस्') }}</a>
                            </td>
                            <form id="delete_relation{{ $i }}"
                                action="{{ route('relation.destroy', $relation) }}" method="POST" class="d-none">
                                @method('DELETE')
                                @csrf
                            </form>
                        </tr>
