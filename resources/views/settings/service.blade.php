...
@section('title', 'सेवाहरु')
@section('menu_open','menu-open')
@section('s_child','block')
@section('setting_service', 'active')
@section('main_content')
    ...
    @if (isset($service))
        <div class="card text-sm mt-1">
            <div class="card-header">
                <div class="card-text">
                    <p class="mb-0">{{ __('सेवाहरु सच्याउने') }}</p>
                </div>
                <div class=" card-body">
            ...
            <table id="example1" class="table table-bordered table-striped">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">{{ __('सेवाहरु') }}</th>
                        <th></th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    @foreach ($services as $service)
                        <tr>
                            ...
                            <td class="text-center">{{ $service->name }}
                            </td>
                            <td class="text-center"><a href="{{ route('service.edit', $service) }}"
                                    class="btn-sm btn-success"><i class="fas fa-edit px-1"></i> {{ __('सच्याउने') }}</a>
                                <a href="#" class="btn-sm btn-danger"
                                    onclick="event.preventDefault();
                                                                                                            document.getElementById('delete_service{{ $i }}').submit();">
                                    <i class="fas fa-trash-alt px-2"></i>{{ __('हटाउनुहोस्') }}</a>
                            </td>
                            <form id="delete_service{{ $i }}"
                                action="{{ route('service.destroy', $service) }}" method="POST" class="d-none">
                                @method('DELETE')
                                @csrf
                            </form>
                        </tr>
    ...
    {{-- modal for adding service status --}}
    <div class="modal fade text-sm" id="modal-lg">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="">{{ __('सेवाहरु थप्नुहोस') }}</h5>
                    <button type=" button"
    ...
    {{-- end of modal for adding service status --}}
@endsection
