    ...
    @if (isset($crop))
        <div class="card text-sm mt-1">
            <div class="card-header">
                <div class="card-text">
                    <p class="mb-0">{{ __('बाली सच्याउने') }}</p>
                </div>
                <div class=" card-body">
        ...
        <div class="card-header my-2">
            <div class="row my-1">
                <div class="col-md-6" style="margin-bottom:-5px;">
                    <p class="">{{ __('अन्न बालीको सुचिहरु') }}</p>
                </div>
                <div class="
                        col-md-6 text-right">
                        <a class="btn text-white btn-sm btn-primary" data-toggle="modal" data-target="#modal-lg">
                            {{ __('बाली थप्नुहोस') }}</a>
                          <a href="{{route('crop-child.index')}}" class="btn text-white btn-sm btn-primary">
                          <i class="fas fa-eye px-1"></i> {{ __('बाली अन्तर्गत हेर्नुहोस') }}</a>
                </div>
            </div>
        </div>
        <!-- /.card-header -->
                ...
                <tbody>
                    ...
                    @foreach ($crops as $crop)
                        <tr>
                            ...
                            <td class="text-center"><a href="{{ route('crop.edit', $crop) }}"
                                    class="btn-sm btn-success"><i class="fas fa-edit px-1"></i> {{ __('सच्याउने') }}</a>
                                <a href="#" class="btn-sm btn-danger"
                                    onclick="event.preventDefault();
                                                                                                            document.getElementById('delete_crop{{ $i }}').submit();">
                                    <i class="fas fa-trash-alt px-2"></i>{{ __('हटाउनुहोस्') }}</a>
                            </td>
                            <form id="delete_crop{{ $i }}"
                                action="{{ route('crop.destroy', $crop) }}" method="POST" class="d-none">
                                @method('DELETE')
                                @csrf
                            </form>
                        </tr>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="">{{ __('अन्न बाली थप्नुहोस') }}</h5>
                    <button type=" button"
