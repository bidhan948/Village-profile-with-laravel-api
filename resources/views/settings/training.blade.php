...
@section('setting_training', 'active')
@section('main_content')
    ...
    @if (isset($training))
        <div class="card text-sm mt-1">
            <div class="card-header">
                <div class="card-text">
                    <p class="mb-0">{{ __('तालिम सच्याउने') }}</p>
                </div>
                <div class=" card-body">
                        ...
                        <div class="row" style="margin-left:-30px;">
                            <div class="col-6">
                                <div class="input-group input-group-sm">
                                    ...
                                    <input type="text" value="{{ $training->name }}" name="name"
            ...
            <table id="example1" class="table table-bordered table-striped">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">{{ __('तालिम') }}</th>
                        <th></th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    @foreach ($trainings as $training)
                        <tr>
                            ...
                            <td class="text-center">{{ $training->name }}
                            </td>
                            <td class="text-center"><a href="{{ route('training.edit', $training) }}"
                                    class="btn-sm btn-success"><i class="fas fa-edit px-1"></i> {{ __('सच्याउने') }}</a>
                                <a href="#" class="btn-sm btn-danger"
                                    onclick="event.preventDefault();
                                                                                                            document.getElementById('delete_training{{ $i }}').submit();">
                                    <i class="fas fa-trash-alt px-2"></i>{{ __('हटाउनुहोस्') }}</a>
                            </td>
                            <form id="delete_training{{ $i }}"
                                action="{{ route('training.destroy', $training) }}" method="POST" class="d-none">
                                @method('DELETE')
                                @csrf
                            </form>
                        </tr>
    ...
    {{-- modal for adding training status --}}
    <div class="modal fade text-sm" id="modal-lg">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="">{{ __('तालिम थप्नुहोस') }}</h5>
                    <button type=" button"
    ...
    {{-- end of modal for adding training status --}}
@endsection
